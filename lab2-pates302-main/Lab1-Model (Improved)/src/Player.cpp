...
{    
    ...
    if(!checkCollision()) 
        gmRef->startMeasurement();           // check collision.  If collision never happened,  
        myPos->removeTail();         // removeTail.  Otherwise, generate new item.    
        gmRef->endMeasurementAndRecord();    // Think about how you'd measure removeTail() computation time.
